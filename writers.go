...
func XMLApiWriteOne(writer *XMLWriter, object *Api, tag string) error {
	...
	if r, ok := object.EngineBackup(); ok {
		XMLEngineBackupInfoWriteOne(writer, r, "engine_backup")
	}
...
func XMLDisplayWriteOne(writer *XMLWriter, object *Display, tag string) error {
	...
	if r, ok := object.VideoType(); ok {
		XMLVideoTypeWriteOne(writer, r, "video_type")
	}
	writer.WriteEnd(tag)
	return nil
}
...
func XMLEngineBackupInfoWriteOne(writer *XMLWriter, object *EngineBackupInfo, tag string) error {
	if object == nil {
		return fmt.Errorf("input object pointer is nil")
	}
	if tag == "" {
		tag = "engine_backup_info"
	}
	writer.WriteStart("", tag, nil)
	if r, ok := object.LastCinderBackup(); ok {
		writer.WriteDate("last_cinder_backup", r)
	}
	if r, ok := object.LastDbBackup(); ok {
		writer.WriteDate("last_db_backup", r)
	}
	if r, ok := object.LastDwhBackup(); ok {
		writer.WriteDate("last_dwh_backup", r)
	}
	if r, ok := object.LastEngineBackup(); ok {
		writer.WriteDate("last_engine_backup", r)
	}
	if r, ok := object.LastGrafanaBackup(); ok {
		writer.WriteDate("last_grafana_backup", r)
	}
	if r, ok := object.LastKeycloakBackup(); ok {
		writer.WriteDate("last_keycloak_backup", r)
	}
	writer.WriteEnd(tag)
	return nil
}

func XMLEngineBackupInfoWriteMany(writer *XMLWriter, structSlice *EngineBackupInfoSlice, plural, singular string) error {
	if plural == "" {
	...
	writer.WriteStart("", plural, nil)
	for _, o := range structSlice.Slice() {
		XMLEngineBackupInfoWriteOne(writer, o, singular)
	}
	writer.WriteEnd(plural)
	return nil
}
...
func XMLLogicalUnitWriteOne(writer *XMLWriter, object *LogicalUnit, tag string) error {
	...
	writer.WriteStart("", tag, attrs)
	if r, ok := object.ActivePaths(); ok {
		writer.WriteInt64("active_paths", r)
	}
	if r, ok := object.Address(); ok {
		writer.WriteCharacter("address", r)
	}
...
func XMLActionWriteOne(writer *XMLWriter, object *Action, tag string) error {
	...
	if r, ok := object.AttachWgt(); ok {
		writer.WriteBool("attach_wgt", r)
	}
...
func XMLVideoTypeWriteOne(writer *XMLWriter, enum VideoType, tag string) {
	if tag == "" {
		tag = "video_type"
	}
	writer.WriteCharacter(tag, string(enum))
...
func XMLVideoTypeWriteMany(writer *XMLWriter, enums []VideoType, plural, singular string) error {
	if plural == "" {
		plural = "video_types"
	}
	if singular == "" {
